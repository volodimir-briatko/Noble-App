...
class Products extends godfather
{
    ...
    public function get_products($filter = array())
    {
        ...
        if (isset($filter['vis'])) $vis_filter = sprintf(" AND vis='%s'", $this
            ->db
            ->real_escape($filter['vis']));
        if (isset($filter['type'])) $type_filter = sprintf(" AND type='%s'", $this
            ->db
            ->real_escape($filter['type']));

        if (isset($filter['sirname']))
        {
            $page_name_filter = sprintf("%s", $this
                ->db
                ->real_escape($filter['sirname']));
        ...
        {
            $club_filter = sprintf("%s", $this
                ->db
                ->real_escape($filter['club']));
        ...
        if (isset($filter['product_ids']))
        {
            ...
            $product_ids_filter = sprintf(" AND id IN (%s)", $this
                ->db
                ->real_escape($product_ids_filter));
        }
        ...
        {
            if (empty($filter['ids_not']))
            {
                $filter['ids_not'] = array(
                    0
                );
            }
        ...
        {
            if (empty($filter['group_present']))
            {
                $filter['group_present'] = array(
                    0
                );
            }
        ...
        {
            if (empty($filter['group_color']))
            {
                $filter['group_color'] = array(
                    0
                );
            }
        ...
        {
            if (empty($filter['shkaf_group']))
            {
                $filter['shkaf_group'] = array(
                    0
                );
            }
    ...
    public function get_products_count($filter = array())
    {
        ...
        if (isset($filter['vis'])) $vis_filter = sprintf(" AND vis='%s'", $this
            ->db
            ->real_escape($filter['vis']));
        if (isset($filter['s_top2'])) $s_top2_filter = sprintf(" AND s_top2>0");
        if (isset($filter['s_top3'])) $s_top3_filter = sprintf(" AND s_top3>0");
        if (isset($filter['product_ids']))
        {
            if (empty($filter['product_ids']))
            {
    ...
    public function insert_product($arr)
    {
        $max = $this
            ->db
            ->result("Select max(`pos`) as `maxid` from `products`");
        $max = $max[0]->maxid;
        $pos = $max + 1;
        $columns = 'pos';
        $values = "'$pos'";
