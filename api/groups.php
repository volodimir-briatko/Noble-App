...
public function get_pages($filter = array())	{
	...
	$page_names_filter = '';
		...
		if(!empty($filter['ids'])){
			...
			$ids_filter = sprintf(" AND id IN (%s)",  $this->db->real_escape($ids_filter));				
		}
	...
	if(isset($filter['inmenu']))
			$inmenu_filter = sprintf(" AND inmenu='%s'", $this->db->real_escape($filter['inmenu']) );

	if(isset($filter['category_id']))
			$category_filter = sprintf(" AND category='%s'", $this->db->real_escape($filter['category_id']) );	
...
$max = $this->db->result("Select max(`pos`) as `maxid` from `groups`");
$max = $max[0]->maxid;
$pos = $max+1; 
		$columns = 'pos';
		$values = "'$pos'";
