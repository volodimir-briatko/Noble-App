...
class Characteristics extends godfather
{
    ...
    public function get_characteristics($filter = array())
    {
        ...
        {
            $club_id_filter = sprintf(" AND club_id='%s'", $this
                ->db
                ->real_escape($filter['club_id']));
        }
    ...
    public function insert_page($arr)
    {
        $max = $this
            ->db
            ->result("Select max(`pos`) as `maxid` from `characteristics`");
        $max = $max[0]->maxid;
        $pos = $max + 1;
        $columns = 'pos';
        $values = "'$pos'";
    ...
    {
        $product_id_filter = '';
        if (isset($filter['product_id'])) $product_id_filter = sprintf(" AND product_id='%s'", $this
            ->db
            ->real_escape($filter['product_id']));
    ...
    {
        $char_id_filter = '';
        if (isset($filter['char_id'])) $char_id_filter = sprintf(" AND characteristic_id='%s'", $this
            ->db
            ->real_escape($filter['char_id']));
