...
class Categories extends godfather
{
    ...
    {
        $where_user_id = sprintf("AND id='%s'", $this
            ->db
            ->real_escape($id));
    ...
    public function get_category($id, $filter = array())
    {
        ...
        if (isset($filter['parent_id'])) $parent_id_filter = sprintf(" AND parent_id='%s'", $this
            ->db
            ->real_escape($filter['parent_id']));
    ...
    public function get_categories($filter = array())
    {
        ...
        {
            $page_name_filter = sprintf("%s", $this
                ->db
                ->real_escape($filter['page_name']));
        ...
        if (!empty($filter['calc'])) $calc_filter = sprintf(" AND calc='%s'", $this
            ->db
            ->real_escape($filter['calc']));

        if (!empty($filter['user_email'])) $user_email_filter = sprintf(" AND user_email='%s'", $this
            ->db
            ->real_escape($filter['user_email']));
        ...
        if (!empty($filter['parent_id'])) $parent_id_filter = sprintf(" AND parent_id='%s'", $this
            ->db
            ->real_escape($filter['parent_id']));
    ...
    public function insert_category($arr)
    {
        $max = $this
            ->db
            ->result("Select max(`pos`) as `maxid` from `categories`");
        $max = $max[0]->maxid;
        $pos = $max + 1;
    ...
    {
        $where = sprintf("category_id='%s'", $this
            ->db
            ->real_escape($id));
    ...
    public function delete_category_product($product_id)
    {
        ...
        if ($product_id > 0)
        {
            ...
            $query = sprintf("DELETE FROM `categories_features` WHERE product_id='%s'", $this
                ->db
                ->real_escape($product_id));
            $this
                ->db
                ->query($query);
        }
        ...
        if (!empty($categories))
        {
            foreach ($categories as $v)
            {
                $value = sprintf("'%s'", $this
                    ->db
                    ->real_escape($v));
                $query = sprintf("INSERT INTO `categories_features` (category_id,product_id) VALUES ($value,'%s') ", $this
                    ->db
                    ->real_escape($product_id));
                $this
                    ->db
