...
public function get_series($filter = array())	{
	...
	$zagolovok_filter = '';
		...
		if(isset($filter['ids'])){
		if(empty($filter['ids'])){$filter['ids'] = array(99999999); }
			$id_filter = " AND id IN (".implode(",", $filter['ids']).")";
			}
			
		
		if(!empty($filter['group_id']))
			$group_filter = sprintf(" AND groupa='%s'",  $this->db->real_escape($filter['group_id']));
			
		if(!empty($filter['vis']))
			$vis_filter = sprintf(" AND vis='%s'",  $this->db->real_escape($filter['vis']));

		if(!empty($filter['zagolovok']))
			$zagolovok_filter = sprintf(" AND zagolovok='%s'",  $this->db->real_escape($filter['zagolovok']));
...
$max = $this->db->result("Select max(`pos`) as `maxid` from `series`");
$max = $max[0]->maxid;
$pos = $max+1; 
