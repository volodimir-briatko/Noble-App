...
if (!empty($_POST['api_key'])){
	...
	*/
	if (!empty($_POST['api_event']) && $_POST['api_event']=='club_password_recovery' && password_verify($api_key, $settings->pdf_file) ){
		...
		if ( !empty( $clubs ) ){
			...
			$godfather->categories->update_category($club->id,array('pass_change'=>$rand_name,'pass_time'=>time() ));
	...
	*/
	if (!empty($_POST['api_event']) && $_POST['api_event']=='club_get' && password_verify($api_key, $settings->pdf_file) ){
		...
		foreach ($clubs as $key => $value) {
			...
			if ($club->parent_id==0){
				...
				$club->url = $device_id;
				$godfather->categories->update_category( $club->id,array('parent_id'=>1,'url'=>$device_id) );
	...
	if (!empty($_POST['api_event']) && $_POST['api_event']=='club_exit' && password_verify($api_key, $settings->pdf_file) ){
		...
		$godfather->categories->update_category( $club->id,array('parent_id'=>0,'url'=>'') );
		echo 'success logout';
	}
	...
	if (password_verify($api_key, $settings->pdf_file) && password_verify($user_pass, $club_autorize->user_pass) && $user_email==$club_autorize->user_email ) {
		...
		if ( !empty($_POST['api_event']) && $_POST['api_event']=='client_add' ){
			...
			if ( ($_FILES['client_foto']['name'])==true ){
				$image1 = $godfather->images->upload_image($_FILES['client_foto'],360);
				if (!empty($image1)) {
			...
			$res_id = $godfather->products->insert_product($client_arr);
			if (!empty($club)){
				$godfather->categories->insert_categories_features(array($club->id),$res_id);
			}
			echo $res_id;
		}
		...
		if (!empty($_POST['api_event']) && $_POST['api_event']=='client_redact' && !empty($_POST['client_id']) ){
			...
			$res_id = $godfather->products->update_product($client_id,$client_arr);
			if (!empty($club) && $res_id){
				$godfather->categories->delete_category_product($client_id);
				$godfather->categories->insert_categories_features(array($club->id),$client_id);
			}
			if ($res_id) echo 1;
		}
		...
		if (!empty($_POST['api_event']) && $_POST['api_event']=='client_delete' && !empty($_POST['client_id']) ){
			$client_id = intval($godfather->prepare_var($_POST['client_id']) );
			$godfather->db->delete_items(array($client_id),'products');
			echo 1;
		}
		...
		if (!empty($_POST['api_event']) && $_POST['api_event']=='client_get' && !empty($_POST['club_id']) ){
			$club_id = intval($godfather->prepare_var($_POST['club_id']) );
			$categories_products = $godfather->categories->get_categories_products($club_id);
			$filter = array();
...
*/
			...
			if (in_array($api_sort,array('dateS','sirname'))){
